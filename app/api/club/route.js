...
async function fetchCreatorCount() {
  const results = []
  let cursor
  do {
  ...
  results.forEach(page => {
    ...
    const cat = p['Categoria Amplify Club']?.select?.name ?? 'Start'
    byCategoria[cat] = (byCategoria[cat] || 0) + 1
  })
  return { total: results.length, byCategoria }
}

async function fetchGmvSummary() {
  try {
    ...
    if (!listRes.ok) return { totalGmv: 0, amplifyRevenue: 0, activeCreators: 0 }
    const { files } = await listRes.json()
    if (!files?.length) return { totalGmv: 0, amplifyRevenue: 0, activeCreators: 0 }

    const relevant = files.filter(f => {
      const m = f.name.match(/(\d{4}-\d{2}-\d{2})_(\d{4}-\d{2}-\d{2})/)
      return m ? m[1] <= untilDate && m[2] >= sinceDate : false
    })
    if (!relevant.length) return { totalGmv: 0, amplifyRevenue: 0, activeCreators: 0 }
    ...
    for (const file of relevant) {
      ...
      rows.slice(1).forEach(r => {
        ...
        if (nome && nome.toLowerCase() !== 'resumo' && parseBRL(r[iGmv]) > 0) {
          activeHandles.add(cleanHandle(nome))
        }
      })
    }

    return { totalGmv, amplifyRevenue: totalCom * 0.10, activeCreators: activeHandles.size }
  } catch {
    return { totalGmv: 0, amplifyRevenue: 0, activeCreators: 0 }
  }
}
...
export async function GET() {
  try {
    ...
    const [{ total, byCategoria }, { totalGmv, amplifyRevenue, activeCreators }] =
      await Promise.all([fetchCreatorCount(), fetchGmvSummary()])

    const result = { total, byCategoria, totalGmv, amplifyRevenue, activeCreators, updatedAt: new Date().toISOString() }
    cache   = result
    cacheAt = Date.now()
