...
export function filterNewBrandStagesByVariant(stages, variant) {
  ...
  return stages.map((stage) => {
    ...
    if (stage.key === 'creative-view') {
      return {
        ...stage,
        value: null,
        conversion: null,
        sourceLabel: 'Sem quebra por variante',
        metricScope: 'unavailable',
      }
    }
    if (stage.key === 'landing-view') {
      return {
        ...stage,
        value: Number(variant.visits) || 0,
        conversion: null,
        sourceLabel: `Entrada identificada · Variante LP · ${label}`,
        eventName: variant.visitSource === 'lp_view' ? 'lp_view · sessao_unica' : 'minimo_comprovado_por_lead',
        metricScope: 'variant',
      }
    }
