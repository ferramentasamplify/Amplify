...
export function buildTikTokDailyGmvTimeline({ from, to, handles } = {}) {
  ...
  if (from?.endsWith("-01")) {
    ...
    while (cursor <= to) {
      ...
      const point = { date: cursor, total: 0, creators: {}, status: snapshot ? "OK" : "DEGRADED", source: snapshot ? "month_to_date_partner_center_snapshot" : "missing_cumulative_snapshot" };
      if (snapshot) {
        const rows = rowMapForSnapshot(snapshot, handleSet);
        for (const handle of handleSet || rows.keys()) {
          const value = Number(rows.get(handle)?.gmv || 0);
  ...
  while (cursor <= to) {
    const day = ledger.byDay.get(cursor);
    const point = { date: cursor, total: 0, creators: {}, status: day?.status || "DEGRADED", source: day?.source || "missing_daily_ledger" };
    if (day?.status === "OK") {
      for (const [handle, metrics] of day.rows.entries()) {
        if (handleSet && !handleSet.has(handle)) continue;
