...
const dailyCsvRowsCache = new Map();
...
function rowsForDailyCsv(filePath) {
  const stat = fs.statSync(filePath);
  const cacheKey = `${filePath}:${stat.mtimeMs}:${stat.size}`;
  const cached = dailyCsvRowsCache.get(cacheKey);
  if (cached) return cached;
  const rows = new Map();
  const rawRows = readCsvRows(filePath);
  const headers = new Set(rawRows.headers || []);
  const hasRequiredColumns = ["author_alias", "sum_cl_pay_amt"].every((header) => headers.has(header));
  for (const row of rawRows) {
    const handle = cleanHandle(row.author_alias);
    if (!handle) continue;
    const metrics = rowMetrics(row);
    if (hasAnyMetric(metrics)) rows.set(handle, metrics);
  }
  Object.defineProperties(rows, {
    rawRowCount: { value: rawRows.length },
    delimiter: { value: rawRows.delimiter || "," },
    hasRequiredColumns: { value: hasRequiredColumns },
  });
  dailyCsvRowsCache.set(cacheKey, rows);
  return rows;
}
...
function listTikTokDailyLedgerFiles() {
  const rawDir = path.join(dataDir(), "creator-daily-ledger", "raw");
  ...
  return months.flatMap((month) => {
    ...
    return fs
      ...
      .filter((name) => /^creator_day__\d{4}-\d{2}-\d{2}\.csv$/.test(name))
      .map((name) => {
        const fullPath = path.join(monthDir, name);
        const stat = fs.statSync(fullPath);
        const day = name.match(/creator_day__(\d{4}-\d{2}-\d{2})\.csv$/)?.[1] || "";
        return {
          path: fullPath,
...
function buildDailyLedger() {
  ...
  const dailyCsvFiles = listTikTokDailyLedgerFiles();
  const signature = `${snapshotSignature(snapshots)}|${dailyCsvFiles.map((file) => `${file.path}:${file.mtimeMs}:${file.size}`).join("|")}`;
  if (dailyLedgerCache?.signature === signature) return dailyLedgerCache.ledger;

  const byDay = new Map();
  for (const file of dailyCsvFiles) {
    byDay.set(file.day, {
      date: file.day,
      status: "OK",
      source: "exact_daily_partner_center_export",
      snapshot: {
        file: file.relativePath,
        fullPath: file.path,
        month: file.month,
        start: file.day,
        endExclusive: addDaysISO(file.day, 1),
        endInclusive: file.day,
        partial: true,
        rows: null,
        totalCount: null,
...
function ensureDailyRows(day) {
  if (!day || day.rows instanceof Map) return day?.rows || new Map();
  if (day.csvPath) {
    day.rows = rowsForDailyCsv(day.csvPath);
    day.rawRowCount = day.rows.rawRowCount ?? null;
    day.csvDelimiter = day.rows.delimiter || null;
    day.hasRequiredColumns = day.rows.hasRequiredColumns !== false;
    if (day.snapshot) {
      day.snapshot.rows = day.rows.size;
      day.snapshot.totalCount = day.rows.size;
    }
    return day.rows;
  }
  return new Map();
...
export function aggregateTikTokDailyLedger({ from, to, handles } = {}) {
  ...
  while (cursor <= requestedTo) {
    ...
    const rows = ensureDailyRows(day);
    if (day.csvPath && (!day.hasRequiredColumns || (Number(day.rawRowCount || 0) > 0 && rows.size === 0))) {
      coverageParts.push({
        date: cursor,
        status: "DEGRADED",
        source: !day.hasRequiredColumns ? "daily_csv_missing_required_columns" : "parsed_empty_daily_csv",
        snapshot: day.snapshot || null,
        beforeSnapshot: day.beforeSnapshot || null,
      });
      cursor = addDaysISO(cursor, 1);
    ...
    for (const [handle, metrics] of rows.entries()) {
  ...
  if (coverageParts.some((part) => part.source === "parsed_empty_daily_csv")) {
    warnings.push("Arquivo diario do Partner Center existe, mas parseou 0 creators com metricas. Resultado bloqueado para evitar OK falso.");
  }
  if (coverageParts.some((part) => part.source === "daily_csv_missing_required_columns")) {
    warnings.push("Arquivo diario do Partner Center sem colunas obrigatorias para GMV/creator. Resultado bloqueado.");
  }
  ...
  while (cursor <= to) {
    ...
    if (day?.status === "OK") {
      ...
      if (day.csvPath && (!day.hasRequiredColumns || (Number(day.rawRowCount || 0) > 0 && rows.size === 0))) {
        ...
        point.source = !day.hasRequiredColumns ? "daily_csv_missing_required_columns" : "parsed_empty_daily_csv";
        warnings.push(`Dia ${cursor} com arquivo diario parseado sem creators validos.`);
      } else {
        for (const [handle, metrics] of rows.entries()) {
          if (handleSet && !handleSet.has(handle)) continue;
