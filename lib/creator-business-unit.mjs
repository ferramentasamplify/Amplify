...
export function buildExitedCreatorRows(exitedCreators, fallbackTopExits = []) {
  const source = Array.isArray(exitedCreators) ? exitedCreators : Array.isArray(fallbackTopExits) ? fallbackTopExits : []
  return source
    .map((creator) => ({
      authorId: String(creator?.authorId || ''),
      alias: String(creator?.alias || ''),
      gmvPrior30d: round(creator?.gmvPrior30d),
    }))
    .sort((left, right) => right.gmvPrior30d - left.gmvPrior30d || left.authorId.localeCompare(right.authorId))
}
...
export function normalizeGastosRows(pages = []) {
  ...
  }).filter((row) => /^\d{4}-\d{2}$/.test(row.month) && row.value > 0)
}
...
export function buildCreatorCostInventory(costs = []) {
  const categories = CATEGORY_DEFINITIONS.map((definition) => {
    const lineItems = costs.filter((row) => (row.category || classifyCreatorCost(row)) === definition.key)
    return { ...definition, amount: round(lineItems.reduce((sum, row) => sum + row.value, 0)), lineItems }
  })
  return {
    totalCost: round(costs.reduce((sum, row) => sum + row.value, 0)),
    lineItemCount: costs.length,
    categories,
  }
}
...
export function buildRetentionAnalytics(daily = [], creators = [], creatorMonths = new Map(), months = [], { coverageFrom = null } = {}) {
  ...
  const monthly = months.map((month) => {
    ...
    const cohortEvents = events.filter((event) => event.date.startsWith(month))
    const cohortCreators = new Set(cohortEvents.map((event) => event.creatorId))
    const cohortReturnedCreators = new Set(cohortEvents.filter((event) => event.returnedLater).map((event) => event.creatorId))
    const observedExits = rows.reduce((sum, row) => sum + (Number(row.exits) || 0), 0)
    const observedReturns = rows.reduce((sum, row) => sum + (Number(row.returns) || 0), 0)
    const exitedGmvPrior30d = round(rows.reduce((sum, row) => sum + (Number(row.exitedGmvPrior30d) || 0), 0))
    return {
      month,
      observedExits,
      observedReturns,
      cohortExitedCreators: cohortCreators.size,
      cohortReturnedCreators: cohortReturnedCreators.size,
      cohortReturnPercent: cohortCreators.size ? round(cohortReturnedCreators.size / cohortCreators.size * 100) : null,
      cohortReturnPercentDisplay: cohortCreators.size >= 10 ? round(cohortReturnedCreators.size / cohortCreators.size * 100) : null,
      exitedCreatorsWithGmv30d: rows.reduce((sum, row) => sum + (Number(row.exitedCreatorsWithGmv30d) || 0), 0),
      exitedGmvPrior30d,
      avgGmvPrior30dPerExit: observedExits ? round(exitedGmvPrior30d / observedExits) : null,
