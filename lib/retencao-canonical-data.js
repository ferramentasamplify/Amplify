...
export function getRetencaoCanonicalPeriod({ from, to, handles } = {}) {
  const useCumulativeSnapshot = startsAtMonthBoundary(from);
  const snapshot = useCumulativeSnapshot
    ? aggregateTikTokSnapshots({ from, to, handles })
    : aggregateTikTokDailyLedger({ from, to, handles });
  ...
  return {
    ...
    source: {
      ...
      strategy: useCumulativeSnapshot ? "server_side_cumulative_creator_snapshots" : "server_side_daily_creator_ledger",
      rule: useCumulativeSnapshot
        ? "Periodos desde o dia 01 usam o corte acumulado oficial do Partner Center. Periodos quebrados usam soma de dias fechados confiaveis. Notion/contratos entram como dimensoes auxiliares, nao substituem dinheiro oficial."
        : "Periodos quebrados usam soma de dias fechados confiaveis do Partner Center. Notion/contratos entram como dimensoes auxiliares, nao substituem dinheiro oficial.",
    },
