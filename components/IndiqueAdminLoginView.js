...
export default function IndiqueAdminLoginView() {
  const router = useRouter();
  const sp = useSearchParams();
  const rawNext = sp.get("next") || "/indiqueeganhe";
  ...
  async function submit(e) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/indiqueeganhe-admin/login", {
        method: "POST",
      ...
      const next = data.next || (rawNext.startsWith("/") && !rawNext.startsWith("/indiqueeganhe/login")
        ? rawNext
        : "/indiqueeganhe");
      router.push(next);
      router.refresh();
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_18%_18%,rgba(37,99,235,.42),transparent_34%),radial-gradient(circle_at_82%_18%,rgba(233,30,99,.22),transparent_28%),linear-gradient(135deg,#06163A_0%,#0A1F4C_45%,#1E3A8A_100%)] text-white flex items-center justify-center px-4 py-10">
      ...
      <div className="relative w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-[11px] font-black uppercase text-white/75 shadow-2xl">
            <span className="h-2 w-2 rounded-full bg-[#2563EB] shadow-[0_0_18px_rgba(37,99,235,.75)]" />
            Acesso unificado
          </div>
          <h1 className="mt-5 text-3xl font-black tracking-tight">Indique e Ganhe</h1>
          <p className="mt-2 text-sm text-white/70">
            Use amplify para o painel completo ou seu usuário para o painel individual.
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-white/20 bg-white p-6 text-[#101828] shadow-[0_30px_90px_rgba(4,19,54,.38)]">
          <form onSubmit={submit} className="space-y-4">
            <label className="block">
              <span className="mb-1.5 block text-[11px] font-bold uppercase text-slate-500">Usuário</span>
              <input
                type="text"
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                placeholder="amplify ou @ do indicador"
                autoFocus
                autoCapitalize="none"
            ...
            <button
              ...
              className="w-full rounded-2xl bg-[linear-gradient(135deg,#2563EB,#1D4ED8)] py-3 text-sm font-black text-white shadow-[0_16px_40px_rgba(37,99,235,.28)] transition-all hover:-translate-y-0.5 hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0"
            >
