    ...
    <div className="econ-wrap">
      ...
      {loading && !data ? <div className="econ-loading"><i /><span>Cruzando aquisicao, formularios e retencao…</span></div> : data && <>
        <section className="metric-grid">
          ...
          <Metric label="CAC medio pago" value={paid.acquiredCac == null ? "—" : money(paid.acquiredCac)} note={`${integer(paid.cohortCreators)} first-touch Meta vinculados a retencao`} tone="amber" />
        ...
        <footer className="econ-method">
          <div><span>Receita, nao lucro</span><p>{data.caveats[1]}</p></div><div><span>Identidade</span><p>{data.methodology.identity} {data.methodology.form}</p></div><div><span>CAC</span><p>{data.caveats[2]}</p></div><div><span>Fontes</span><p>{data.caveats[3]}</p></div><div><span>Atualizacao</span><p>Snapshot gerado em {new Date(data.generatedAt).toLocaleString("pt-BR")}. Cobertura de {date(data.coverage.from)} a {date(data.coverage.to)}.</p></div>
        </footer>
      </>}
    </div>
