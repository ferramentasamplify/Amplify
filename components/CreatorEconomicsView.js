...
const ratio = (value) => value == null ? "—" : `${Number(value).toLocaleString("pt-BR", { maximumFractionDigits: 2 })}x`
...
export default function CreatorEconomicsView() {
  const [from, setFrom] = useState("2026-03")
    ...
    <div className="econ-wrap">
      ...
      {loading && !data ? <div className="econ-loading"><i /><span>Cruzando aquisicao, formularios e retencao…</span></div> : data && <>
        <section className="metric-grid">
          ...
          <Metric label="CAC medio pago" value={paid.acquiredCac == null ? "—" : money(paid.acquiredCac)} note={`${integer(paid.cohortCreators)} creators pagos adquiridos na coorte`} tone="amber" />
          <Metric label="LTV / CAC pago" value={ratio(paid.ltvCac)} note={`LTV observado medio ${paid.avgObservedLtv == null ? "—" : money(paid.avgObservedLtv)}`} tone="rose" />
        </section>

        <section className="paid-callout">
          ...
          <div><span>CPL Meta</span><strong>{paid.leadCpl == null ? "—" : money(paid.leadCpl)}</strong><small>{paid.leads == null ? "Sem resultado retornado" : `${integer(paid.leads)} resultados Meta`}</small></div>
        ...
        <section className="econ-panel source-panel">
          ...
          <div className="source-table-wrap"><table className="source-table"><thead><tr><th>Origem</th><th>Creators ativos</th><th>Entraram</th><th>Com formulario</th><th>GMV</th><th>Receita Amplify</th><th>LTV medio observado</th></tr></thead><tbody>{data.sourceBreakdown.map((row) => <tr key={row.key}><td><button onClick={() => { setSource(row.key); setPage(1) }}>{row.label}</button></td><td>{integer(row.creators)}</td><td>{integer(row.entered)}</td><td>{percent(row.formMatchRate)}</td><td>{money(row.gmv)}</td><td><strong>{money(row.estimatedAmplifyRevenue)}</strong></td><td>{money(row.avgObservedLtv)}</td></tr>)}</tbody></table></div>
        </section>
...
function FragmentRow({ creator, expanded, onToggle }) {
  return <>
    <tr className="creator-row">
      ...
      <td>{money(creator.totals.estimatedAmplifyRevenue)}</td>
