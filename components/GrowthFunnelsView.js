...
function StageBox({ stage, color, primary = false }) {
  ...
  return (
    <div className={`stage-box ${empty ? "empty" : ""} ${primary ? "primary" : ""} ${stage.gmv != null ? "has-money" : ""}`} style={{ "--accent": color }}>
      ...
      {stage.gmv != null && <div className="stage-money" title={`GMV registrado: ${Number(stage.gmv).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}`}>
        <span>GMV registrado</span><b>{fmtMoney(stage.gmv)}</b>
        <small>{fmt(stage.gmvCount)} com GMV · {fmtPct(stage.gmvCoverage)}</small>
      </div>}
      {stage.amplifyGain != null && <div className="stage-gain"><span>Amplify · 1%</span><b>{fmtMoney(stage.amplifyGain)}</b></div>}
    </div>
  );
}
    ...
    <section className="audience-panel" style={{ "--audience": accent }}>
      <div className="audience-head">
        ...
        <div className="audience-stats">
          ...
          <div className="money-stat"><span>GMV final registrado</span><strong>{fmtMoney(audience.totals.gmv)}</strong><small>{audience.totals.gmv == null ? "fonte não conectada" : `${fmt(audience.totals.gmvCount)} com GMV`}</small></div>
          <div className="gain-stat"><span>Ganho Amplify · 1%</span><strong>{fmtMoney(audience.totals.amplifyGain)}</strong><small>{audience.totals.amplifyGain == null ? "aguardando GMV" : "sobre o GMV final"}</small></div>
        </div>
      </div>
      ...
      <div className="channel-list">
        {sortedChannels.map((channel) => {
          ...
          const finalStage = channel.stages.at(-1);
          const finalConversion = channel.leads > 0 && finalStage?.value != null
            ? (finalStage.value / channel.leads) * 100
            : null;
          return (
              ...
              <div className="channel-name" style={{ "--channel": tone }}>
                ...
                <div className="channel-summary">
                  ...
                  <span className="channel-final">
                    ...
                    {finalStage?.value != null && <i>{fmt(finalStage.value)} {audience.key === "creators" ? "agenciados" : "fechados"}</i>}
                  </span>
                  {finalStage?.gmv != null && <span className="channel-money">
                    <em>GMV final</em><b>{fmtMoney(finalStage.gmv)}</b>
                    <i>Amplify 1% · {fmtMoney(finalStage.amplifyGain)}</i>
                  </span>}
                </div>
              </div>
      ...
      <div className="page-wrap">
        <header className="hero">
          <div className="hero-copy">
            ...
            <p>De onde cada lead entra, como avança pela máquina, quanto GMV carrega e o ganho final da Amplify — com Creators e Marcas em funis separados.</p>
          </div>
