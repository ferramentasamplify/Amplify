...
function TreeNode({ node, accent, depth = 0, defaultOpen = false }) {
  ...
  return (
    <div className={`tree-node depth-${Math.min(depth, 4)} ${open ? "open" : ""}`}>
      <article className="tree-card" style={{ "--tree-accent": accent }}>
        <button
      ...
      {expandable && open && <div className="tree-children">
        {node.children.map((child) => <TreeNode key={child.id} node={child} accent={accent} depth={depth + 1} />)}
      </div>}
    </div>
  );
}
      ...
      <div className="page-wrap">
        ...
        {loading && !data ? <Loading /> : data && <>
          ...
          <div className="audience-stack">
            ...
            {showBrands && <AudiencePanel audience={audiences.brands} tree={data.hierarchy?.brands} metaSource={data.metaSource} accent="#FF765F" />}
          </div>
