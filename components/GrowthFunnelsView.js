...
function GMVPanel({ audience, accent }) {
  if (audience.totals.gmv == null) return null;
  const stages = audience.stages.filter((stage) => stage.gmv != null);
  const channels = audience.channels
    .map((channel) => ({ ...channel, gmv: channel.stages.at(-1)?.gmv || 0 }))
    .filter((channel) => channel.gmv > 0)
    .sort((a, b) => b.gmv - a.gmv);
  const maxStage = Math.max(...stages.map((stage) => stage.gmv), 1);
  const maxChannel = Math.max(...channels.map((channel) => channel.gmv), 1);
  return (
    <section className="gmv-panel" style={{ "--gmv-accent": accent }}>
      <div className="gmv-head">
        <div><span className="eyebrow">Leitura financeira</span><h3>Fluxo de GMV registrado</h3><p>GMV registrado na coorte, da entrada ao agenciamento.</p></div>
        <div className="gmv-result">
          <div><span>GMV final</span><strong>{fmtMoney(audience.totals.gmv)}</strong><small>{fmt(audience.totals.gmvCount)} registros preenchidos</small></div>
          <div className="amplify-result"><span>Ganho Amplify · 1%</span><strong>{fmtMoney(audience.totals.amplifyGain)}</strong><small>calculado sobre o GMV final</small></div>
        </div>
      </div>
      <div className="gmv-charts">
        <div className="gmv-chart">
          ...
          {stages.map((stage) => <div className="gmv-row" key={stage.key}>
            <span>{stage.label}</span><b>{fmtMoney(stage.gmv)}</b>
            <div><i style={{ width: `${Math.max(2, (stage.gmv / maxStage) * 100)}%` }} /></div>
            <small>{fmt(stage.gmvCount)} com GMV · {fmtPct(stage.gmvCoverage)}</small>
          </div>)}
        </div>
        <div className="gmv-chart">
          <h4>GMV final por canal</h4>
          {channels.map((channel) => <div className="gmv-row channel-gmv-row" key={channel.key} style={{ "--bar-color": TONES[channel.tone] || TONES.slate }}>
            <span>{channel.short || channel.label}</span><b>{fmtMoney(channel.gmv)}</b>
            <div><i style={{ width: `${Math.max(2, (channel.gmv / maxChannel) * 100)}%` }} /></div>
            <small>Amplify 1% · {fmtMoney(channel.gmv * 0.01)}</small>
          </div>)}
        </div>
      </div>
    </section>
