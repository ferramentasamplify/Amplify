...
function LpVariantComparison({ variants }) {
  if (!variants) return null;
  return <section className={`lp-variant-comparison ${variants.available ? "available" : "unavailable"}`}>
    <header>
      <div><span className="eyebrow">Comparação da LP</span><h3>LP antiga x Headline A x Headline B</h3><p>Mesma campanha e mesma URL. Origem continua sendo o canal; a variante fica separada.</p></div>
      <small>{variants.available ? variants.reference : variants.error || "fonte indisponivel"}</small>
    </header>
    <div className="lp-variant-grid">
      {(variants.rows || []).map((row) => <article key={row.key} className={`lp-variant-card ${row.key}`}>
...
function NewBrandFunnelPanel({ funnel }) {
  ...
  return (
    <section className="new-brand-funnel" style={{ "--new-brand": "#F6B84B" }}>
      ...
      <LpVariantComparison variants={funnel.live?.variants} />
