...
function Funnel({ name, stages, gmvs, labels = stageLabels, compact = false }) {
  ...
  const directConversions = [
    { key: "contact-meeting", from: 0, to: 3, fromLabel: "Primeiro contato", toLabel: "Reuniao realizada" },
    { key: "meeting-win", from: 3, to: 5, fromLabel: "Reuniao realizada", toLabel: "Agenciado" },
  ];
  return <div className={`funnel ${compact ? "compact" : ""}`} style={{ "--person": people[name].color }}>
    ...
    <aside className="direct-rail" aria-label="Conversoes diretas">
      <span className="direct-title">Conversao direta</span>
      {directConversions.map((item) => <div className="direct-link" key={item.key}>
        <i className="direct-arrow" aria-hidden="true">→</i>
        <div><span>{item.fromLabel}</span><b>{pct(percent(stages[item.to], stages[item.from]))}</b><span>{item.toLabel}</span></div>
      </div>)}
    </aside>
  </div>;
}
