...
cleanup() {
  docker exec -u 0 "$CID" rm -f /tmp/build-growth-funnels.js /tmp/growth-funnels-live.json /tmp/creds.json /tmp/export.log >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
docker exec "$CID" sh -lc 'set -e; n8n export:credentials --all --decrypted --output=/tmp/creds.json >/tmp/export.log 2>&1; FUNNEL_SNAPSHOT_OUTPUT=/tmp/growth-funnels-live.json node /tmp/build-growth-funnels.js'
